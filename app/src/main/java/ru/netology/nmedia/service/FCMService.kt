...
class FCMService : FirebaseMessagingService() {
    ...
    override fun onMessageReceived(message: RemoteMessage) {
        ...
        val action = Action.values().find { it.key == serializedAction } ?: null
        if (action == null) {
            Handler(Looper.getMainLooper()).post {
                Toast.makeText(
                    this,
                    "Incorrect notification element",
                    Toast.LENGTH_LONG
                ).show()
    ...
    private fun handleNewPostAction(serializedContent: String) {
        val postContent = gson.fromJson(serializedContent, PostFromNotification::class.java)
